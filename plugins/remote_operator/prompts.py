...
def _build_task_block(task_description: str) -> str:
    ...
    return (
        ...
        "每个工具的必填参数缺失即视为调用失败。\n"
        "9. 【提权方式唯一且强制】需要 root 权限时，只能且必须将 exec_command 的 "
        "use_sudo 参数设为 true，绝不允许在 command 里自行拼接任何提权程序。"
        "禁止在 command 中出现 sudo、pkexec、su、doas、gksu、gksudo 等任何提权命令，"
        "也禁止用 script -q -c \"pkexec ...\" 之类的方式绕过限制。\n"
        "   - 后端服务通常以 systemd 服务运行，没有图形会话、没有 polkit 认证代理、"
        "也没有可交互 TTY。此时 pkexec 会卡在「Authenticating as: ...」永久挂起，"
        "导致命令超时、会话占用、整个任务失败。\n"
        "   - 唯一可用的提权路径是 use_sudo=true：后端会通过「echo 密码 | sudo -S」"
        "在子管道内传递密码，不依赖 TTY、不依赖 polkit、不依赖图形会话。\n"
        "   - 若 use_sudo=true 返回「后端未启用 sudo 执行」或「后端未配置 sudo 密码」，"
        "说明该服务器后端未配置 sudo，此时应停止提权尝试，向用户说明需要在后端 "
        "config.toml 的 [sudo] 节配置 enabled=true 与 password 后重试，"
        "不要改用 pkexec/su/doas 等「替代方案」——它们在 systemd 环境下同样会挂起。\n\n"
        "【输出要求】\n"
        ...
        "3. 【代称会被自动打回重写】后端对 finish_task 的 result 启用了自包含校验："
        "一旦检测到「上面就是」「如上所示」「见上文」「如前所述」等指代 result 之外"
        "内容的措辞，本次 finish_task 会被直接拒绝（打回），你必须重新调用 finish_task "
        "提交一份自包含的 result。重复违规会耗尽重写次数并被强制结束。"
        "因此务必在第一次提交时就把所有用户需要的内容原样写进 result。\n"
        "4. 如果用户要求「完整命令」「原样输出」「给我命令」等，你必须在 result 中"
        "把对应的命令文本原样完整写出（含完整路径、参数、管道等），不做改写、不加分页、不折叠。\n"
        "5. 若一条命令的输出很长，仍应完整保留，可用代码块包裹以便阅读，但不得删减、截断或只写摘要。\n"
        "6. 一句话总结：result 不是对已发生操作的点评，而是用户能看到的全部。"
...
def _build_tool_reference() -> str:
    ...
    return (
        ...
        "2. tool-list_files(server_id, path)\n"
        "   列出指定服务器上某目录的内容，返回文件与子目录列表。\n"
        "   - server_id: str, 必填 —— 目标服务器 ID（通过 tool-list_servers 获取）。\n"
        "   - path: str, 必填 —— 要列出的目录绝对路径。\n\n"
        "3. tool-read_file(server_id, path, encoding?)\n"
        "   读取指定服务器上某文件的内容。大文件会被后端截断，关注关键部分。\n"
        "   - server_id: str, 必填 —— 目标服务器 ID。\n"
        "   - path: str, 必填 —— 文件绝对路径。\n"
        "   - encoding: str, 可选（默认 \"utf-8\"）—— 文件编码。\n\n"
        "4. tool-write_file(server_id, path, content, encoding?)\n"
        ...
        "   - encoding: str, 可选（默认 \"utf-8\"）—— 文件编码。\n\n"
        "5. tool-edit_file(server_id, path, edits, encoding?)\n"
        ...
        "   - encoding: str, 可选（默认 \"utf-8\"）—— 文件编码。\n\n"
        "6. tool-delete_file(server_id, path, recursive?)\n"
        "   删除指定服务器上的文件或目录。破坏性操作，执行前务必确认路径正确。\n"
        "   - server_id: str, 必填 —— 目标服务器 ID。\n"
        "   - path: str, 必填 —— 要删除的路径。\n"
        "   - recursive: bool, 可选（默认 false）—— 是否递归删除目录，"
        "删除目录时需设为 true。\n\n"
        "7. tool-create_terminal(server_id, remark, shell?, cwd?)\n"
        ...
        "8. tool-exec_command(server_id, terminal_id, command, timeout?, use_sudo?)\n"
        "   在指定终端中执行命令，保留会话上下文。根据返回的 exit_code 判断成功与否，"
        "失败时查看 stderr 分析原因。返回结果中 success 字段 = (exit_code == 0)。\n"
        "   - server_id: str, 必填 —— 目标服务器 ID。\n"
        "   - terminal_id: str, 必填 —— 终端 ID（通过 tool-create_terminal 获取）。\n"
        "   - command: str, 必填 —— 要执行的 shell 命令。"
        "严禁在此参数中包含 sudo/pkexec/su/doas/gksu/gksudo 等任何提权程序，"
        "也禁止用 script -q -c \"pkexec ...\" 之类的方式绕过；"
        "需要 root 权限时只能用下面的 use_sudo=true。\n"
        "   - timeout: int, 可选（默认 30，上限 120）—— 单条命令超时秒数。\n"
        "   - use_sudo: bool, 可选（默认 false）—— 是否以 sudo 执行。"
        "如需 root 权限必须设此参数为 true，由后端通过 sudo -S 安全提权。"
        "后端通常以 systemd 服务运行，无 polkit 代理与可交互 TTY，"
        "因此 pkexec/su/doas 等其他提权方式会永久挂起导致超时失败。\n\n"
        "9. tool-list_terminals(server_id)\n"
        "   列出指定服务器上所有活跃的终端会话，包含 terminal_id、备注、工作目录等。"
        "用于在多个终端间定位或确认状态。\n"
        "   - server_id: str, 必填 —— 目标服务器 ID。\n\n"
        "10. tool-close_terminal(server_id, terminal_id)\n"
